import numpy as np
from axon import Variable
from axon.core import Function, get_array_module, cupy
...
def im2col(x, kernel_size, stride=1, pad=0, to_matrix=True):
    ...
    Args:
        x (`axon.Variable` or `ndarray`): Input variable of shape
            `(N, C, H, W)`
    ...
    Returns:
        `axon.Variable`: Output variable. If the `to_matrix` is False, the
...
def im2col_array(img, kernel_size, stride, pad, to_matrix=True):
    ...
    N, C, H, W = img.shape # 输入数据的形状
    KH, KW = pair(kernel_size) # 卷积核的高和长
    SH, SW = pair(stride) # 步幅
    PH, PW = pair(pad) # 填充
    OH = get_conv_outsize(H, KH, SH, PH) # 输出数据的高
    OW = get_conv_outsize(W, KW, SW, PW) # 输出数据的长
    ...
    if xp != np:
        ...
        img = np.pad(img,
                     ...
                     mode='constant', constant_values=(0,)) # 填充
        col = np.ndarray((N, C, KH, KW, OH, OW), dtype=img.dtype) # 输出数据的形状

        for j in range(KH):
            j_lim = j + SH * OH # 卷积核的高
            for i in range(KW):
                i_lim = i + SW * OW # 卷积核的长
                col[:, :, j, i, :, :] = img[:, :, j:j_lim:SH, i:i_lim:SW] # 从输入数据中取出卷积核大小的数据

    if to_matrix:
        col = col.transpose((0, 4, 5, 1, 2, 3)).reshape((N * OH * OW, -1)) # 转换成二维矩阵
